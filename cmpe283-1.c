...
/*
 * Flags for Secondary and Tertiary Controls
 */
 ...
 */
struct capability_info secondary_proc_based[28] =
	{
...
void detect_vmx_features(void)
{
	...
	/* primary_proc_based controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS, lo, hi);
	pr_info("primary_proc_based Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(primary_proc_based, 22, lo, hi);

	/* Primary Proc based VME Controls: IA32_VMX_PROCBASED_CTLS (SDM)
	 * Bit position 31: Activate secondary controls
	 ...
	 * operates as if all the secondary processor-based VM-execution controls were also 0.
	 */
	if (hi & (1 << (31)))
	{
		secondary = 1;
	...
	/* Primary Proc based VME Controls: IA32_VMX_PROCBASED_CTLS (SDM)
	 * Bit position 17: Activate tertiary controls
	 * This control determines whether the tertiary processor-based
	 * VM-execution controls are used. If this control is 0, the logical processor
	 * operates as if all the tertiary processor-based VM-execution controls were also 0.
	 */
	if (hi & (1 << (17)))
	{
		tertiary = 1;
	...
	/* secondary_proc_based controls */
	if (secondary)
	{
		rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
		pr_info("secondary_proc_based Controls MSR: 0x%llx\n",
				(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(secondary_proc_based, 28, lo, hi);
	}

	/* tertiary_proc_based controls */
	if (tertiary)
	{
		rdmsr(IA32_VMX_PROCBASED_CTLS3, lo, hi);
		pr_info("tertiary_proc_based Controls MSR: 0x%llx\n",
				(uint64_t)(lo | (uint64_t)hi << 32));
	...
	pr_info("vmx_exit Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmx_exit, 17, lo, hi);

	/* vmx_entry controls */
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("vmx_entry Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
